...
#
# Global PCA variant of SliceGPT
# This script adds --use-global-pca flag to test single rotation matrix approach
...
def slicing_arg_parser(interactive: bool = True) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--final-orientation",
        type=str,
        default="pca",  # Changed default to 'pca' for global PCA experiments
        choices=["random", "pca"],
    ...
    parser.add_argument('--wandb-project', type=str, default="slicegpt-global-pca", help="wandb project name.")
    ...
    parser.add_argument(
        "--use-global-pca",
        action="store_true",
        help="Use single global PCA instead of per-layer PCA (experimental)",
    )
...
def slicing_main(args: argparse.Namespace) -> None:
    ...
    if args.use_global_pca:
        logging.info("*** USING GLOBAL PCA MODE ***")
    ...
    # Choose between global PCA and per-layer PCA
    if args.use_global_pca:
        logging.info("Using GLOBAL PCA rotation...")
        rotate.rotate_and_slice_global_pca(
            model_adapter, 
            train_loader, 
            scheduler, 
            apply_mask=(args.cal_dataset != "c4"),
            final_orientation=args.final_orientation,
        )
    else:
        logging.info("Using per-layer PCA rotation...")
    ...
    if args.save_dir:
        ...
        # Add suffix to distinguish global PCA models
        suffix = "_global_pca" if args.use_global_pca else ""
        sliced_model_name = sliced_model_dir / f'{pathlib.Path(args.model).name}_{args.sparsity}{suffix}.pt'
    ...
    pca_type = "Global PCA" if args.use_global_pca else "Per-layer PCA"
    logging.info(f'After rotating and slicing ({pca_type}): {dataset_ppl:.4f}')
    wandb.log({"sliced_ppl": dataset_ppl, "pca_type": pca_type})
